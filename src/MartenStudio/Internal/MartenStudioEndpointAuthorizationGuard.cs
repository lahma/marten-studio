...
/// <summary>
/// Refuses to start an application whose Marten Studio endpoints nothing authorizes.
/// </summary>
/// <remarks>
/// <para>
/// Marten Studio's pages are fully mutating and add no authentication of their own: they can read and
/// edit every document in every Marten store in this process, archive event streams, control the
/// projection daemon, apply schema changes and run SQL. An open one is therefore a serious exposure
/// rather than an information leak, and the mistake that opens it is a mapping call that says nothing.
/// So a mapping that says nothing is the thing that fails, at startup, where it is cheap.
/// </para>
/// <para>
...
/// </para>
/// <para>
/// Registered by <c>AddMartenStudio</c>. Adding it without mapping it leaves nothing marked and nothing
...
internal sealed class MartenStudioEndpointAuthorizationGuard : IHostedLifecycleService
{
    ...
    public MartenStudioEndpointAuthorizationGuard(
        MartenStudioMappedEndpoints mappedEndpoints,
    ...
    private void Verify(List<Endpoint> endpoints)
    {
        ...
        Dictionary<string, (MartenStudioEndpointMarker Marker, List<string> Routes)> unguarded = new(StringComparer.Ordinal);
        foreach (Endpoint endpoint in endpoints)
        {
            MartenStudioEndpointMarker? marker = endpoint.Metadata.GetMetadata<MartenStudioEndpointMarker>();
    ...
    private static string BuildMessage(Dictionary<string, (MartenStudioEndpointMarker Marker, List<string> Routes)> unguarded)
    {
        StringBuilder message = new();
        foreach ((MartenStudioEndpointMarker marker, List<string> routes) in unguarded.Values)
        {
            if (message.Length > 0)
            {
            ...
            message.AppendLine("Marten Studio is mapped with no authorization. Its pages can read and edit every document in every Marten store in this process, archive event streams, control the projection daemon, apply schema changes and run SQL, so Marten Studio refuses to start rather than serve that by accident. Say which you meant:")
