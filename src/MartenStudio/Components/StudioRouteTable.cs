...
/// <summary>
/// Matches dashboard URLs against the route templates declared by the studio page components.
/// The standalone dashboard root uses this instead of the built-in Blazor router so the studio
/// can be served from a configurable <see cref="MartenStudioOptions.Path"/>; the
/// built-in router can only match the compile-time "/marten" templates against the browser URL.
/// </summary>
internal static class StudioRouteTable
{
    private static readonly Lazy<List<RouteEntry>> Routes = new(BuildRouteTable);

    /// <summary>
    /// Matches like <see cref="Match(string)"/> but retries with the studio path prefix
    ...
    /// in <see cref="StudioLink.ToDashboardRelativePath"/> cannot distinguish from an
    /// interactive leaf, so a failed match gets a second chance without the prefix.
    /// </summary>
    internal static RouteData? Match(string dashboardRelativePath, MartenStudioOptions options)
    {
    ...
    /// applying the same prefix-strip retry as <see cref="Match(string, MartenStudioOptions)"/>.
    ...
    internal static string ResolveLeaf(string dashboardRelativePath, MartenStudioOptions options)
    {
        ...
        if (StudioLink.TryStripDashboardPrefix(normalized, options, out string stripped) && Match(stripped) is not null)
        {
            return stripped;
        }
    ...
    /// <summary>
    /// Matches a dashboard-root-relative path ("" for the studio root, no leading slash, as
    /// produced by <see cref="StudioLink.ToDashboardRelativePath"/>) against the studio
    ...
    {
        dashboardRelativePath = StudioLink.NormalizeRelativePath(dashboardRelativePath);
    ...
    private static List<RouteEntry> BuildRouteTable()
    {
        ...
        foreach (Type type in typeof(StudioRouteTable).Assembly.GetTypes())
        {
            ...
            foreach (RouteAttribute attribute in type.GetCustomAttributes<RouteAttribute>())
            {
                ...
                if (!template.StartsWith(MartenStudioOptions.DefaultPath, StringComparison.OrdinalIgnoreCase)
                    || (template.Length > MartenStudioOptions.DefaultPath.Length && template[MartenStudioOptions.DefaultPath.Length] != '/'))
                {
                    ...
                    throw new InvalidOperationException(
                        ...
                        $"'{MartenStudioOptions.DefaultPath}'. All dashboard page templates must start with '{MartenStudioOptions.DefaultPath}'.");
                }

                string remainder = template.Substring(MartenStudioOptions.DefaultPath.Length);
